...
def sensitivity(b,c,Na):
    ...
    D = 0.5*rho*V**2*S*(CD0 + CDA*alfa**2) + Df
    #plt.figure()
    ...
    Tmax = []
    for v_inf in V:
        T = BEM(R,cprop,w,angle_of_pitch,v_inf,rho)
        Tmax.append(T)
    #plt.plot(V,Tmax,label='Thrust Available')
    #plt.legend()
    print 'Thrust Max = ',Tmax[0]
...
###Nominal Parameters
c_nominal = 0.7 #based on iteration above
b_nominal = 7.0 #Based on iterations above
Na_nominal = 1.0

M_nominal = sensitivity(b_nominal,c_nominal,Na_nominal)

##Trade Study on Chord
M_vec = []
#The chord length 
c_vec = np.linspace(0.2,2.0,10) #feet
b = b_nominal
Na = Na_nominal
ctr = 0
for c in c_vec:
    M_vec.append(sensitivity(b,c,Na))
M_vec = np.asarray(M_vec)

plt.plot(100.*(c_vec-c_nominal)/c_nominal,100*(M_vec-M_nominal)/M_nominal,label='Chord')

##Variation on attack stores
Na_vec = np.asarray([0,1,2])
M_vec = []
c = c_nominal
b = b_nominal
ctr = 0
for Na in Na_vec:
    M_vec.append(sensitivity(b,c,Na))
M_vec = np.asarray(M_vec)
plt.plot(100.*(Na_vec-Na_nominal)/Na_nominal,100*(M_vec-M_nominal)/M_nominal,label='Attack Stores')

#Wingspan is minimum 4 feet but we can change this
b_vec = np.linspace(2,14,10)
##Trade Study on Wingspan
M_vec = []
c = c_nominal
Na = Na_nominal
ctr = 0
...
plt.plot(100.*(b_vec-b_nominal)/b_nominal,100*(M_vec-M_nominal)/M_nominal,label='Wingspan')
