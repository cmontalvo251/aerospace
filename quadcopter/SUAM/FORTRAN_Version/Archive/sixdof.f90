...
module BODYDATATYPES
  ...
  type BODYSTRUCTURE
     ...
     real*8 :: C_TAU = 0                              ! Body aero parameter
     ...
     real*8 :: OMEGAVEC(4,1) = 0                      ! Body aero parameter
     real*8 :: THRUSTVEC(4,1) = 0                     ! Body aero parameter
     real*8 :: MUVEC(4,1) = 0                         ! Body aero parameter
...
PROGRAM SIXDOF
 ...
 ! Open Time Simulation Output Files
  ...
  open(unit=92,file='State.OUTPUT')
  rewind(92)
  ...
  do i=1,npts
     ...
     nominaltime = T
     nominalstate(1:20) = BODY%STATE(1:20)
...
SUBROUTINE Derivatives(T,BODY)
  ...
  if (BODY%AEROOFFON .eq. 1) then
     ...
     do idx = 1,4
        ...
        C3F(idx) = BODY%MUVEC(idx,1)*sigmaF*omegaF   ! replaced sigma with force
        TDBLDOTVEC(idx) = omegaF*(C1F(idx) + C2F(idx) + C3F(idx))
        BODY%THRUSTVEC(idx,1) = TVEC(idx)
     end do

     BODY%OMEGAVEC = sqrt(BODY%THRUSTVEC/BODY%KT)
     sumomega = sum(BODY%OMEGAVEC)

     !!! Make sure angular velocities of rotor does not go beyond the limit
     if (sumomega .ge. BODY%OMEGAMAX*4) then
        do j = 1,4
     ...
     omegar = BODY%OMEGAVEC(1,1) - BODY%OMEGAVEC(2,1) + BODY%OMEGAVEC(3,1) - BODY%OMEGAVEC(4,1)
     ...
     BODY%MXAERO = Gammavec(1,1) + (BODY%LPHI12*(TVEC(1) - TVEC(2)) + BODY%LPHI34*(TVEC(4) - TVEC(3)))
     BODY%MYAERO = Gammavec(2,1) + BODY%LTHETA12*(TVEC(1) +TVEC(2) - TVEC(3) -TVEC(4))
     BODY%MZAERO = Gammavec(3,1) + bquad*(BODY%OMEGAVEC(1,1)**2 - BODY%OMEGAVEC(2,1)**2 + BODY%OMEGAVEC(3,1)**2 - BODY%OMEGAVEC(4,1)**2)
  else
     BODY%FXAERO = 0
     BODY%FYAERO = 0
     BODY%FZAERO = 0
...
SUBROUTINE Echo_Data(BODY)
  ...
  open(unit=25,file='Parameters.Check',iostat=openflag)
  ...
  if (BODY%MODNO .eq. 3) then
     ...
     write(25,*) 'Quadcopter Aero Parameter: ', BODY%C_TAU
...
SUBROUTINE Load_Data(BODY)
  ...
  read(unit=94,fmt=*,iostat=readflag) BODY%C_TAU
